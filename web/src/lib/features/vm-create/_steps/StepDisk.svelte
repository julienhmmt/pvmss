...
<div class="grid gap-4">
	<label class="grid gap-1 text-sm font-medium">
		...
		{#if form.node !== '' && storagesOnNode.length === 0}
			<span class="text-xs text-destructive">No approved storage on node {form.node}</span>
		{/if}
	</label>

	<label class="grid gap-1 text-sm font-medium">
		Size (GB) <span class="text-xs font-normal text-muted-foreground">1–2048</span>
