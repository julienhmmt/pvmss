...
<Dialog open={showForm} size="lg" labelledBy="cloudinit-file-form-title" onClose={onCancel}>
	<h2 id="cloudinit-file-form-title" class="mb-4 text-lg font-semibold">{editingId ? m['cloudinit.files.edit']() : m['cloudinit.files.new']()}</h2>
	<form onsubmit={(e) => { e.preventDefault(); onSubmit(); }} class="grid gap-4">
		<FormField label={m['cloudinit.files.label']()} required>
		...
		<FormField label={m['cloudinit.files.content']()} hint={m['cloudinit.files.vendorHint']()} required>
			{#snippet children({ id, describedBy, invalid })}
				<Textarea
					{id}
					{describedBy}
