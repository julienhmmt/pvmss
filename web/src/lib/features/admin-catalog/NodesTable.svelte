	...
	<table class="pv-responsive-table text-sm">
		...
		<tbody>
			{#each nodes as node (node.name)}
				...
				<tr class="border-t border-border" data-testid="node-row" data-node-name={node.name}>
					<td class="px-4 py-3 font-mono font-medium" data-label={m['common.name']()}>
						{node.name}
					</td>
					<td class="px-4 py-3" data-label={m['common.status']()}>
						<StatusDot tone={nodeStatusTone(node.status)} label={nodeStatusLabel(node.status)} />
					</td>
					<td class="px-4 py-3 text-muted-foreground" data-label={m['common.vms']()}>
						{node.vmCount}
					</td>
					<td class="px-4 py-3" data-label={m['common.cpu']()}>
						<NodeUsageBar
							value={node.cpuUsage}
							label={m['admin.nodes.cpuLabel']({ cores: node.cpuCores, percent: cpuPct })}
						/>
					</td>
					<td class="px-4 py-3" data-label={m['common.memory']()}>
						<NodeUsageBar
