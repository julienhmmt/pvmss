...
interface StatusStore {
	proxmoxConnection: StatusIndicator;
	clusterHealth: StatusIndicator;
	updateProxmoxConnection(status: StatusIndicator['status'], tooltip?: string): void;
	updateClusterHealth(status: StatusIndicator['status'], tooltip?: string): void;
}
...
function createStatusStore(): StatusStore {
	...
	return {
		...
		updateProxmoxConnection(status: StatusIndicator['status'], tooltip?: string) {
...
function getStatusTooltip(status: StatusIndicator['status'], name: string): string {
	switch (status) {
		case 'connected':
