...
export async function createSnapshot(vmid: number, name: string, description = ''): Promise<void> {
	await api.post(`/api/v1/vms/${vmid}/snapshots`, { name, description });
}

export async function deleteSnapshot(vmid: number, name: string): Promise<void> {
	await api.delete(`/api/v1/vms/${vmid}/snapshots/${name}`);
}

export async function rollbackSnapshot(vmid: number, name: string): Promise<void> {
