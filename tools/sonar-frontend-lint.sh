...
# Run ESLint on frontend/ and web/ (including .svelte files) and convert
# the results to SonarQube generic issue format for import as external issues.
#
...
cd frontend || exit 1
bun run lint -- -f json 2>/dev/null > /tmp/eslint-frontend.json
cd "$REPO_ROOT" || exit 1
convert_eslint_to_sonar ".sonar/frontend-eslint.json" /tmp/eslint-frontend.json "$REPO_ROOT/frontend"
...
rm -f /tmp/eslint-frontend.json /tmp/eslint-web.json
