...
# Format Markdown, TypeScript, and Svelte files with Prettier for PVMSS project
echo "Formatting Markdown, TypeScript, and Svelte files with Prettier..."
...
total_count=$((md_count + ts_count + svelte_count))

if [ "$total_count" -eq 0 ]; then
    echo "⚠️  No Markdown, TypeScript, or Svelte files found in project"
    exit 0
fi
...
output=$(docker run --rm -v .:/workspace -w /workspace node:lts-alpine sh -c "npm install -g prettier prettier-plugin-svelte && prettier --write '**/*.{md,ts,tsx,svelte}' --ignore 'node_modules/**' --ignore '.svelte-kit/**' --ignore 'build/**' --plugin prettier-plugin-svelte 2>&1" || true)

if [ $? -eq 0 ]; then
    echo "✅ Prettier formatting completed successfully!"
    
    # Show only changed files (filter out unchanged and installation noise)
    changed_files=$(echo "$output" | grep -E '\.(md|ts|tsx|svelte)' | grep -v 'unchanged' | grep -v 'node_modules' | grep -v '.svelte-kit' | grep -v 'build')
