...
- [x] **TO CHECK** admin/nodes : il faut un bouton à côté de chaque node pour activer/désactiver le node qu'on laisse à l'utilisateur. On peut déjà le faire dans les paramètres avancés en tant qu'administrateur mais il faut un bouton plus facile d'accès pour les admin dans la page admin/nodes
...
- [x] **TO CHECK** admin/pool utilisateurs : erreur dans le browser 'uncaught typeError: cannot read properties of undefined (reading 'slice') at 23.pGpztlGH.js:2:1544 / at Array.map (<anonymous>) DTeh809U.js
- [x] **TO CHECK** admin/cloud-init : mettre un bouton pour activer ou désactiver le sftp
- [x] **TO CHECK** admin/cloud-init : avoir un statut (config)
- [x] **TO CHECK** user/createVM : error 400 en sélectionnant un profil : on a pas le choix du réseau donc ça plante
- [x] **TO CHECK** user/createVM : error 500 post <https://pvmss-beta.domain.local/api/v1/vms> en voulant créer une vm en mode 'avancé' : POST request returned error status 400 for /nodes/pve1/qemu message: parameter verification failed: errors: ide2 invalid format - format error. ide2.file: unable to parse volume ID
...
- [x] **TO CHECK** user/createVM : cloud-init si les admin pvmss n'ont pas activé l'option alors il ne faut pas proposer l'option dans le formulaire de création de vm
