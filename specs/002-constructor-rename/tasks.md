...
- [ ] T006 [P] Rename reusable helper constructors from `NewErrorHelper`, `NewFormSession`, `NewTemplateDataWithOptions`, `NewMessageHelper`, `NewInputSanitizer`, `NewValidationHelper`, and `NewCSSHandler` to `Make...` in `backend/handlers/errors.go`, `backend/handlers/form_session.go`, `backend/handlers/template_data.go`, `backend/handlers/sanitize.go`, `backend/handlers/validation.go`, and `backend/handlers/css.go`. **Note**: `NewHandlerContext` is explicitly excluded from this rename — it is a deprecated wrapper scheduled for deletion in spec `002-remove-deprecated-functions` (replaced by `HandlerContextWith`, not renamed).
