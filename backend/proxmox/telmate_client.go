...
// Client is a custom wrapper around the Telmate Proxmox API client, enhancing it with timeouts, caching, and simplified authentication.
type Client struct {
	*px.Client
...
// GetPVEAuthCookie returns the Proxmox auth cookie.
func (c *Client) GetPVEAuthCookie() string { return c.PVEAuthCookie }

// GetCSRFPreventionToken returns the CSRF prevention token.
func (c *Client) GetCSRFPreventionToken() string { return c.CSRFPreventionToken }

// SetTimeout updates both the HTTP client and the stored timeout.
...
// NOTE: RestyClient and its methods are defined in resty_client.go
