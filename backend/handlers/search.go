...
// SearchPageHandler renders the Vue SPA shell for the search page.
// All search logic is handled by the Vue frontend via /api/v1/search/vms.
func (h *SearchOptimizedHandler) SearchPageHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	ctx.Log.Debug().Msg("Rendering Vue search page shell")
	renderVueShell(w, r, ctx, "Search VMs")
}
