...
func TestCreateManaged_RoleProvisionedOnce(t *testing.T) {
	cluster.ResetFake()
	t.Cleanup(cluster.ResetFake)
	admin := auth.Identity{Username: "admin", IsAdmin: true}
	client := cluster.Fake{}

	first, err := pools.CreateManaged(context.Background(), admin, client, nil, "default", "carol", "")
	if err != nil {
		t.Fatalf("first CreateManaged: %v", err)
	}
	second, err := pools.CreateManaged(context.Background(), admin, client, nil, "default", "dave", "")
	if err != nil {
		t.Fatalf("second CreateManaged: %v", err)
	}
	if first.PoolName != "pvmss-carol" || second.PoolName != "pvmss-dave" {
...
func TestCreateManaged_ValidationAndDuplicateBeforeProvisioning(t *testing.T) {
	cases := []struct {
		name    string
		wantErr error
	}{
		{name: "", wantErr: pools.ErrInvalidName},
		{name: "-leading", wantErr: pools.ErrInvalidName},
		{name: "trailing-", wantErr: pools.ErrInvalidName},
		{name: "UPPER", wantErr: pools.ErrInvalidName},
		{name: "has_underscore", wantErr: pools.ErrInvalidName},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cluster.ResetFake()
			_, err := pools.CreateManaged(context.Background(), auth.Identity{Username: "admin", IsAdmin: true}, cluster.Fake{}, nil, "default", tc.name, "")
	...
	client := cluster.Fake{}
	admin := auth.Identity{Username: "admin", IsAdmin: true}
	if _, err := pools.CreateManaged(context.Background(), admin, client, nil, "default", "carol", ""); err != nil {
		t.Fatalf("seed CreateManaged: %v", err)
	}
	before := cluster.FakeCalls()
	if _, err := pools.CreateManaged(context.Background(), admin, client, nil, "default", "carol", ""); !errors.Is(err, pools.ErrAlreadyExists) {
		t.Fatalf("duplicate error = %v, want ErrAlreadyExists", err)
	}
	if calls := cluster.FakeCalls(); len(calls) != len(before) {
		t.Fatalf("duplicate made %d new calls, want none", len(calls)-len(before))
	}
}

func TestCreateManaged_NonAdminIsRejectedBeforeClusterCalls(t *testing.T) {
	cluster.ResetFake()
	_, err := pools.CreateManaged(context.Background(), auth.Identity{Username: "alice"}, cluster.Fake{}, nil, "default", "carol", "")
...
// TestCreateManaged_RegistersManagedPool verifies that a successful
// CreateManaged records the prefixed pool name as managed in the store.
//
//nolint:paralleltest // serial: shared fake fixtures
func TestCreateManaged_RegistersManagedPool(t *testing.T) {
	cluster.ResetFake()
	t.Cleanup(cluster.ResetFake)
	admin := auth.Identity{Username: "admin", IsAdmin: true}
	client := cluster.Fake{}
	...
	if _, err := pools.CreateManaged(context.Background(), admin, client, st, "default", "team-y", ""); err != nil {
		t.Fatalf("CreateManaged: %v", err)
	}
	managed, err := st.IsPoolManaged(context.Background(), "default", "pvmss-team-y")
	if err != nil {
		t.Fatalf("IsPoolManaged: %v", err)
	}
	if !managed {
		t.Fatal("pool not recorded as managed after CreateManaged")
	}
}
...
func TestCreateManaged_GeneratesPasswordAndPrefixesPool(t *testing.T) {
	...
	if creds.Comment != "Alice pool" {
		t.Errorf("Comment = %q, want %q", creds.Comment, "Alice pool")
	}
