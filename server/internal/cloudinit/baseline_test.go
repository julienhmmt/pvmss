...
//
//nolint:gocyclo,funlen // table-driven golden test: each case is an independent assertion
func TestBuildVendorData_Golden(t *testing.T) {
	...
	tests := []struct {
		...
		wantErr error
		// check is a positive assertion on the merged document; failures
		// print the document so the golden can be inspected in the test log.
		...
		},
		{
			...
			check: func(t *testing.T, doc string) {
				...
				// User package added, baseline's agent preserved.
				if !strings.Contains(doc, "qemu-guest-agent") {
				...
				// User scalar (write_files) appears.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The merged document is itself a valid #cloud-config.
