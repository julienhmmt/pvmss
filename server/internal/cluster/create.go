...
type Creator interface {
	...
	// task's UPID. The clone is asynchronous like CreateVM;
	...
	// TaskStatus reads a task's current state live. Returns
...
// SourceVMID is the Proxmox VMID of the template; SourceNode
...
// the cloned VM (always the actor's own, never client-supplied).
// TargetNode is reserved for future cross-node clone support (currently always empty, clone
// stays on SourceNode).
...
// biosOVMF is the Proxmox bios value selecting UEFI firmware. Shared between the real create
// path and the fake dataset so both
// recognize the same trigger value.
const biosOVMF = "ovmf"

// VMSpec is the fully-resolved specification of a VM to create. Every field is already
// validated and resolved server-side
// the pool is the actor's own, never client-supplied.
//
// BIOS, Machine, and TPM carry the UEFI/TPM 2.0 options.
...
// DiskSpec is the VM's single initial disk (multi-disk is).
...
// TaskStatus is a live snapshot of an asynchronous task.
