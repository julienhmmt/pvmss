...
const catalogTestCluster = "catalog-test"
...
func TestSetNodeEnabled_EnableAndDisable(t *testing.T) {
	...
	if err := st.SetNodeEnabled(ctx, catalogTestCluster, "pve-node-01", true); err != nil {
	...
	if err := st.SetNodeEnabled(ctx, catalogTestCluster, "pve-node-01", false); err != nil {
...
func TestCatalogNodesEnabled_MultipleSortedByName(t *testing.T) {
	...
	for _, name := range []string{"node-zeta", "node-alpha", "node-mid"} {
...
func TestSetStorageEnabled_EnableAndDisable(t *testing.T) {
	...
	if err := st.SetStorageEnabled(ctx, catalogTestCluster, "local-lvm", "pve-node-01", true); err != nil {
	...
	if storages[0].Name != "local-lvm" {
		t.Errorf("name = %q, want local-lvm", storages[0].Name)
	}
	if storages[0].Node != "pve-node-01" {
	...
	if err := st.SetStorageEnabled(ctx, catalogTestCluster, "local-lvm", "pve-node-01", false); err != nil {
...
func TestSetBridgeEnabled_EnableAndDisable(t *testing.T) {
	...
	if err := st.SetBridgeEnabled(ctx, catalogTestCluster, "pve-node-01", "vmbr0", true); err != nil {
	...
	if bridges[0].Name != "vmbr0" {
		t.Errorf("name = %q, want vmbr0", bridges[0].Name)
	}
	if bridges[0].Node != "pve-node-01" {
	...
	if err := st.SetBridgeEnabled(ctx, catalogTestCluster, "pve-node-01", "vmbr0", false); err != nil {
...
func TestSetISOEnabled_EnableAndDisable(t *testing.T) {
	...
	if err := st.SetISOEnabled(ctx, catalogTestCluster, "pve-node-01", "local", "debian-12.iso", true); err != nil {
	...
	if isos[0].Node != "pve-node-01" {
		t.Errorf("node = %q, want pve-node-01", isos[0].Node)
	}
	if isos[0].Storage != "local" {
	...
	if err := st.SetISOEnabled(ctx, catalogTestCluster, "pve-node-01", "local", "debian-12.iso", false); err != nil {
...
func TestInsertProfile_AndList(t *testing.T) {
	...
	values := store.ProfileValues{Label: "Small", CPUCores: 2, MemoryMB: 4096, DiskGB: 20, Bus: "scsi"}
...
func TestSetProfileEnabled_SuccessAndNotFound(t *testing.T) {
	...
	values := store.ProfileValues{Label: "Medium", CPUCores: 4, MemoryMB: 8192, DiskGB: 40, Bus: "scsi"}
...
func TestUpdateProfile_SuccessAndNotFound(t *testing.T) {
	...
	values := store.ProfileValues{Label: "Original", CPUCores: 2, MemoryMB: 2048, DiskGB: 10, Bus: "scsi"}
...
func TestDeleteProfile_SuccessAndNotFound(t *testing.T) {
	...
	values := store.ProfileValues{Label: "Delete Me", CPUCores: 1, MemoryMB: 1024, DiskGB: 5, Bus: "scsi"}
...
func TestProfileExists_TrueAndFalse(t *testing.T) {
	...
	values := store.ProfileValues{Label: "Exists", CPUCores: 1, MemoryMB: 1024, DiskGB: 5, Bus: "scsi"}
