...
func TestAuthCoverage_CreateToken_InvalidJSON(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/tokens", strings.NewReader("{bad"))
	req.Header.Set("Content-Type", "application/json")
...
func TestAuthCoverage_ChangePassword_InvalidJSON(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/password", strings.NewReader("{bad"))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(cookie)
	rec := httptest.NewRecorder()
	handler.ChangePassword(rec, req)
...
func TestAuthCoverage_ChangePassword_WrongOldPassword(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/password", strings.NewReader(`{"oldPassword":"wrong-old","newPassword":"newpass12"}`))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(cookie)
	rec := httptest.NewRecorder()
	handler.ChangePassword(rec, req)
...
func TestAuthCoverage_ServeClusters_NoStoreReturnsDefault(t *testing.T) {
	...
	if len(clusters) != 1 || clusters[0].Name != "default" {
...
func TestAuthCoverage_Require_Unauthenticated(t *testing.T) {
	handler := newAuthHandler(t)
	called := false
	guarded := handler.Require(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
	}))
	req := httptest.NewRequest(http.MethodGet, "/api/v1/vms/default/100", nil)
	rec := httptest.NewRecorder()
...
func TestAuthCoverage_RequireAdmin_Unauthenticated(t *testing.T) {
	handler := newAuthHandler(t)
	called := false
	guarded := handler.RequireAdmin(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
	}))
...
func TestAuthCoverage_CreateToken_EmptyLabelReturns400(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/tokens", strings.NewReader(`{"label":"","scope":"read"}`))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(cookie)
	rec := httptest.NewRecorder()
	handler.CreateToken(rec, req)
