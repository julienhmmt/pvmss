...
func adminMux(handler *httpapi.AdminCatalog, auth *httpapi.Auth) *http.ServeMux {
	...
	mux.Handle("DELETE /api/v1/admin/bridges/{cluster}/{node}/{name}", guard(http.HandlerFunc(handler.ServeBridgeDelete)))
	...
	mux.Handle("DELETE /api/v1/admin/isos/{cluster}/{node}/{storage}/{file}", guard(http.HandlerFunc(handler.ServeISODelete)))
