...
func TestAdminPools_CreateAndListAsAdmin(t *testing.T) {
	...
	if created.Name != "newteam" || created.Total != 0 || created.Running != 0 || created.Stopped != 0 {
...
func newAdminPoolsHandler(t *testing.T) (*httpapi.AdminPools, *httpapi.Auth) {
	...
	handler := httpapi.NewAdminPools(authHandler, client, projection, client, newAdminStore(t), worker, slog.Default())
