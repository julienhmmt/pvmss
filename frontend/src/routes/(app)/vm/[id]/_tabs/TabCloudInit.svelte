<script lang="ts">
	...
	const customYamlRows = $derived(
		Math.min(40, Math.max(12, customYaml.split('\n').length + 1)),
	);

	// Whether there is real custom cloud-config content (beyond the empty stub).
	const hasCustomYamlContent = $derived(
			...
			{:else if !customYamlEditable}
				...
				{#if hasCustomYamlContent}
					...
					<Textarea
						...
						rows={customYamlRows}
						readonly
						class="max-h-[60vh] resize-y overflow-auto font-mono text-xs opacity-70"
					/>
				{:else}
					<p class="rounded-md border border-yellow-500/30 bg-yellow-500/5 px-3 py-2 text-xs text-yellow-700 dark:text-yellow-400">
