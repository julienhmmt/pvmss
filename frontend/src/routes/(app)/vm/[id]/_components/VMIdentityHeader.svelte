...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
	...
	{#if config}
		<div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
			<span class="font-mono">VMID {config.vmid}</span>
			<span class="hidden sm:inline">•</span>
			<span>Node: <span class="font-medium text-foreground">{config.node}</span></span>
			{#if (metrics?.status ?? config.status) === 'running' && config.uptime}
				<span class="hidden sm:inline">•</span>
				<span>Uptime: <span class="font-medium text-foreground">{Math.floor(config.uptime / 3600)}h {Math.floor((config.uptime % 3600) / 60)}m</span></span>
			{/if}
			{#if config.tags}
				<span class="hidden sm:inline">•</span>
